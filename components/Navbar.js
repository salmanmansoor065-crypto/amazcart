        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 28 }}>
          <Link href="/" style={{ textDecoration: 'none' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              <img src="https://amazcart.vercel.app/logo.svg" alt="AmazCart" style={{ width: 32, height: 32, borderRadius: 8 }} />
          ...
          {isStore && (
            <div style={{ display: 'flex', gap: 2 }}>
              {CATEGORIES.map(cat => (
                <button key={cat} onClick={() => onCategoryChange?.(cat)} style={{ background: activeCategory === cat ? '#FFF0EB' : 'transparent', color: activeCategory === cat ? '#C94A1A' : '#666', border: 'none', padding: '6px 13px', borderRadius: 8, fontSize: 14, fontWeight: activeCategory === cat ? 700 : 500, cursor: 'pointer', transition: 'all 0.15s' }}>
                  {cat}
                </button>
              ))}
            </div>
